...
class SignInNotifyScript:
    ...
    def get_notified_sign_in_ids(self) -> set:
        """获取已发送过签到通知的活动ID"""
        notifications = self.db.query(ActivityNotification.act_id).filter(
            ActivityNotification.notification_type == NotificationType.SIGN_IN.value
        ).all()
    ...
    def get_participants_with_sign_in(self, act_id: int) -> List[Dict]:
        """
        获取有签到时间的参与者
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. 获取已发送过签到通知的活动ID
            notified_ids = self.get_notified_sign_in_ids()
            self.log(f"已有 {len(notified_ids)} 个活动发送过签到通知")
            ...
            for activity in activities:
                ...
                if act_id in notified_ids:
                    self.log(f"活动 {act_name} (ID: {act_id}) 已发送过签到通知，跳过")
                    continue
                ...
                try:
                    ...
                    # 从数据库获取有签到时间的参与者
                    participants = self.get_participants_with_sign_in(act_id)

                    if participants:
                        self.log(f"  -> 找到 {len(participants)} 个有签到记录的参与者")
                        ...
                        # 发送签到通知
                        notify_result = self.notification_service.send_sign_in_notification(
                            activity_info,
                            participants
                        )
...
def run_script(test_mode: bool = False) -> Dict[str, Any]:
    ...
    db = SessionLocal()
    try:
        script = SignInNotifyScript(db, test_mode)
        return script.run()
    finally:
        db.close()
